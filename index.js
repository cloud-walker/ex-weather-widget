...
$(function() {
  return new WeatherWidget(context);
});


},{"./library/weather":7,"jquery":2}],6:[function(require,module,exports){
var $, WeatherApi;
...
WeatherApi = (function() {
  ...
  WeatherApi.prototype.address = 'http://api.openweathermap.org/data/2.5/weather';

  WeatherApi.prototype.get = function(opts) {
    var data;
    if (opts == null) {
      opts = {};
    }
    data = {
      lat: opts.latitude || 0,
      lon: opts.longitude || 0
    };
    return $.getJSON(this.address, data, function(res) {
      return $(window).trigger('openweathermap-data-found', res);
    });
  };
...
var $, WeatherApi, WeatherWidget, gmaps, moment, tuc, wiMap;
...
WeatherWidget = (function() {
  function WeatherWidget(context) {
    this.target = context.find('.weather.widget');
    this.api = new WeatherApi();
    this.initListeners();
    this.fetchPosition();
  }

  WeatherWidget.prototype.fetchPosition = function() {
    return navigator.geolocation.getCurrentPosition((function(_this) {
      return function(pos) {
        return _this.target.trigger('position-fetched', pos);
      };
    })(this));
  };

  WeatherWidget.prototype.loadMap = function(opts) {
    return gmaps.load((function(_this) {
      return function(google) {
        var context, latLng, map, marker;
        context = _this.target.find('.map')[0];
  ...
  WeatherWidget.prototype.initFrontFace = function(res) {
    var footer, target;
    target = this.target.find('.front.face');
    console.log('initFrontFace', res);
    target.find('.temperature').text((Math.round(tuc.k2c(res.main.temp))) + "°");
    target.find('.mood').text(res.weather[0].description);
    target.find('.location').text(res.name + ", " + res.sys.country);
    target.find('.month').text(moment().format('MMM'));
    target.find('.day').text(moment().format('DD'));
    footer = target.find('footer');
    footer.find('.wind .value').text(res.wind.speed + "m/s");
    footer.find('.humidity .value').text(res.main.humidity + "%");
    footer.find('.sun .value').text((100 - res.clouds.all) + "%");
    console.log(res.weather[0].id, wiMap[res.weather[0].id]);
    return target.find('header i').removeClass().addClass(wiMap[res.weather[0].id]).text('');
  };

  WeatherWidget.prototype.initListeners = function() {
    $(window).on('openweathermap-data-found', (function(_this) {
      return function(e, res) {
        return _this.initFrontFace(res);
      };
    })(this));
    return this.target.on('flip', function(e, flip) {
      if (flip) {
        return $(this).addClass('flipped');
      } else {
        return $(this).removeClass('flipped');
      }
    }).on('position-fetched', (function(_this) {
    ...
    })(this)).on('click', '.front .icon', function() {
      return $(this).trigger('flip', true);
    }).on('click', '.back .icon', function() {
      return $(this).trigger('flip', false);
    });
  };
...
},{"./api":6,"./wi-map":8,"google-maps":1,"jquery":2,"moment":3,"temp-units-conv":4}],8:[function(require,module,exports){
module.exports = {
